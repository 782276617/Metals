...
<div class="cont">
  ...
  <div class="settab"> 
	...
	<form name="queryForm" id="queryForm" action="<%=basePath%>admin/goods/goods_list.htm" method="post">
    ...
    <div class="brandtable">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="brand_table">
        <tr style="background:#2A7AD2;height:30px; color:#FFF">
          ...
          <td width="10%">所属分类</td>
          <td width="7%">商品状态</td>
          <td width="7%" align="center">是否促销</td>
        ...
        <tr>
         <td align="left">
          ...
          <td align="center"><img onclick="ajax_update('${list.id}','store_recommend',this)" src="<%=basePath%>resources/system/manage/blue/images/${list.isvouch==1?'true.png':'false.png'}" width="25" height="21" border="0" style="cursor:pointer;" title="特别推荐后在首页显示"/></td>
          <td align="center"><img onclick="ajax_update('${list.id}','goods_status',this)"  src="<%=basePath%>resources/system/manage/blue/images/weig.jpg"  border="0" style="cursor:pointer;" title="可编辑"/> </td>
          ...
          <td class="ac8" align="center"><a href="<%=basePath%>admin/goods/goods_edit.htm?id=${list.id}&currentPage=${data.currPageNo}" >编辑</a><a href="<%=basePath%>home/goods_product.htm?goodsId=${list.id}" target="_blank">查看</a></td>
        </tr>
      	</c:forEach>
	 ...
	 <a href="<%=basePath%>admin/goods/goods_list.htm?currentPage=1&goods_name=${goods_name}&goods_class=${goods_class}&store_recommend=${store_recommend}">首页</a>
	 
	 <c:if test="${data.currPageNo > 1}">
			 <a href="<%=basePath%>admin/goods/goods_list.htm?currentPage=${data.currPageNo-1}&goods_name=${goods_name}&goods_class=${goods_class}&store_recommend=${store_recommend}">上一页</a>
	 </c:if>
	 
	 <c:forEach var="i"  begin="${data.firstPage}" end="${data.lastPage>8?data.firstPage+7:data.lastPage}"  step="1" >
					<a class="${data.currPageNo==i?'this':''}" href="<%=basePath%>admin/goods/goods_list.htm?currentPage=${i}&goods_name=${goods_name}&goods_class=${goods_class}&store_recommend=${store_recommend}">${i}</a>
	 </c:forEach>
	 	<c:if test="${data.currPageNo!=data.lastPage}">
			<a href="<%=basePath%>admin/goods/goods_list.htm?currentPage=${data.currPageNo+1}&goods_name=${goods_name}&goods_class=${goods_class}&store_recommend=${store_recommend}">下一页</a>
	 	</c:if>
		<a href="<%=basePath%>admin/goods/goods_list.htm?currentPage=${data.lastPage}&goods_name=${goods_name}&goods_class=${goods_class}&store_recommend=${store_recommend}">末页</a>
	   </div>
    </div>
  </form>
</div>
